...
Tabs.STab = Tabs.MainTab:Tab({ Title = "传送建筑物", Icon = "zap" })
...
Tabs.eTab = Tabs.MainTab:Tab({ Title = "自瞄", Icon = "zap" })
...
local function aimAtWolf()
    ...
    for _, wolf in ipairs(workspace:GetChildren()) do
        if wolf.Name == "Wolf" and wolf:FindFirstChild("HumanoidRootPart") then
            local distance = (wolf.HumanoidRootPart.Position - humanoidRootPart.Position).Magnitude
    ...
    if closestWolf then
        -- 计算瞄准方向（只水平旋转）
        local wolfPosition = closestWolf.HumanoidRootPart.Position
        local lookAt = Vector3.new(wolfPosition.X, humanoidRootPart.Position.Y, wolfPosition.Z)
...
Tabs.SpeedTab:Toggle({
    ...
    Callback = function(state)
        ...
        if state then
            -- 开启瞄准
            aimingCoroutine = coroutine.create(startAiming)
...
local function aimAtWolf()
    ...
    for _, wolf in ipairs(workspace:GetChildren()) do
        if wolf.Name == "Bunny" and wolf:FindFirstChild("HumanoidRootPart") then
            local distance = (wolf.HumanoidRootPart.Position - humanoidRootPart.Position).Magnitude
            if distance < closestDistance then
                closestWolf = wolf
                closestDistance = distance
